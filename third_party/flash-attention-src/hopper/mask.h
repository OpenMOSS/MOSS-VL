...
struct Mask {
    ...
    int const* const cross_kv_boundary;
        ...
        if constexpr (!Causal_mask && !Local_mask) {
            ...
            if constexpr (!SwapAB) {
                    ...
                    if (cross_kv_boundary != nullptr) {
                        ...
                        for (int m = 0; m < size<0>(tSrS_rowcol); ++m) {
                            int const row_idx = !PackGQA
                                ? get<Row>(tScS_rowcol(m, _0{})) + m_block * kBlockM
                                :  __shfl_sync(0xffffffff, mma_m_idx, m % kMmaThreadsPerRow, kMmaThreadsPerRow);
                            int col_limit_right = (row_idx < seqlen_q)
                                ? cross_kv_boundary[row_idx] - n_block * kBlockN - thread_col_offset
                                : seqlenk_col_limit;
                            if constexpr (Seqlenk_mask) {
                                col_limit_right = std::min(col_limit_right, seqlenk_col_limit);
                            }
                            #pragma unroll
                            for (int n = 0; n < size<1>(tSrS_rowcol); ++n) {
                                if (int(get<Col>(t0ScS_rowcol(_0{}, n))) >= col_limit_right) { tSrS_rowcol(m, n) = -INFINITY; }
                ...
                if constexpr (Causal_mask) {
                    ...
                    if (cross_kv_boundary != nullptr) {
                        // Staircase backward (SwapAB): for each (m, n) in the transposed accumulator,
                        // actual_q_row = m_block*kBlockM + t0ScS_row[m] + thread_row_offset
                        // actual_k_col = n_block*kBlockN + t0ScS_col[n] + thread_col_offset
                        // Mask out if actual_k_col >= cross_kv_boundary[actual_q_row].
                        #pragma unroll
                        for (int m = 0; m < size<0>(tSrS_rowcol); ++m) {
                            int const actual_q_row = int(get<Row>(t0ScS_rowcol(m, _0{}))) + thread_row_offset + m_block * kBlockM;
                            int const row_kv_limit = (actual_q_row >= 0 && actual_q_row < seqlen_q)
                                ? cross_kv_boundary[actual_q_row]
                                : 0;
                            int const col_limit_for_row = row_kv_limit - n_block * kBlockN - thread_col_offset;
                            #pragma unroll
                            for (int n = 0; n < size<1>(tSrS_rowcol); ++n) {
                                if (int(get<Col>(t0ScS_rowcol(_0{}, n))) >= col_limit_for_row) { tSrS_rowcol(m, n) = -INFINITY; }
                            }
                        }
                    } else {
