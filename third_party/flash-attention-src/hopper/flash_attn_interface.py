...
def _prepare_cross_kv_boundary(
    cross_kv_boundary,
    *,
    expected_shape=None,
    expected_ndim=None,
    ...
    if cross_kv_boundary is None:
        return None
    if not isinstance(cross_kv_boundary, torch.Tensor):
        raise TypeError("cross_kv_boundary must be a torch.Tensor")
    if cross_kv_boundary.dtype != torch.int32:
        raise TypeError("cross_kv_boundary must have dtype torch.int32")
    if expected_shape is not None and tuple(cross_kv_boundary.shape) != tuple(expected_shape):
        raise ValueError(
            f"cross_kv_boundary must have shape {tuple(expected_shape)}, "
            f"got {tuple(cross_kv_boundary.shape)}"
        )
    if expected_ndim is not None and cross_kv_boundary.dim() != expected_ndim:
        raise ValueError(
            f"cross_kv_boundary must be {expected_ndim}D, got {cross_kv_boundary.dim()}D"
        )
    if device is not None and cross_kv_boundary.device != device:
        raise ValueError(
            f"cross_kv_boundary must be on device {device}, got {cross_kv_boundary.device}"
        )
    if causal:
        raise ValueError("cross_kv_boundary cannot be used together with causal attention")
    return maybe_contiguous(cross_kv_boundary)
...
def _flash_attn_forward(
    ...
    sm_margin: int = 0,
    cross_kv_boundary: Optional[torch.Tensor] = None,
    ...
    expected_boundary_shape = (q.shape[0],) if cu_seqlens_q is not None else q.shape[:2]
    cross_kv_boundary = _prepare_cross_kv_boundary(
        cross_kv_boundary,
...
def _flash_attn_backward(
    ...
    cross_kv_boundary = maybe_contiguous(cross_kv_boundary)
...
def _flash_attn_backward_fake(
    ...
    if is_varlen_k:
        if max_seqlen_k is None:
            raise ValueError("max_seqlen_k must be provided if cu_seqlens_k is provided")
    ...
    is_local = ((window_size_left >= 0 or window_size_right >= 0) and not is_causal) or cross_kv_boundary is not None
...
def setup_context(ctx, inputs, output):
    ...
    cross_kv_boundary = inputs[-1]
    ctx.save_for_backward(q, k, v, out, softmax_lse, cross_kv_boundary)
...
def _backward(ctx, dout, *grads):
    q, k, v, out, softmax_lse, cross_kv_boundary = ctx.saved_tensors
...
def flash_attn_qkvpacked_func(
    ...
    Arguments:
        ...
        cross_kv_boundary: optional int32 tensor of shape (batch_size, seqlen_q). Each value is the
            number of KV tokens visible to that query row. Values are per-row prefix lengths
            and may be non-monotonic, including right-padding rows with value 0. Values outside
            [0, seqlen_k] are interpreted as if clamped to that range.
...
def flash_attn_varlen_func(
    ...
    cross_kv_boundary, when provided, must be an int32 tensor of shape (total_q,) with
    per-row KV prefix lengths in each cu_seqlens_q segment. Values are interpreted
    independently per query row, may be non-monotonic, and are logically clamped to the
    corresponding sequence's [0, seqlen_k] range.
    """
...
def flash_attn_with_kvcache(
    ...
    Arguments:
        ...
        cross_kv_boundary: optional int32 tensor. If cu_seqlens_q is None, shape is
            (batch_size, seqlen_q); otherwise shape is (total_q,). Values are
            logical KV prefix lengths after cache_leftpad is removed and after any
            new k/v are appended, and are logically clamped to [0, seqlen_k].
...
def get_scheduler_metadata(
    ...
    cross_kv_boundary = _prepare_cross_kv_boundary(
        cross_kv_boundary,
        expected_shape=(batch_size, max_seqlen_q) if cu_seqlens_q is None else None,
        expected_ndim=1 if cu_seqlens_q is not None else None,
