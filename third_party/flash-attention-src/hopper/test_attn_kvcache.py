...
def _flash_attn_with_kvcache_out_lse(**kwargs):
    result = flash_attn_interface.flash_attn_with_kvcache(**kwargs)
    return result[0], result[1]


def _ones_descale(batch_size, nheads_kv):
    return torch.ones((batch_size, nheads_kv), dtype=torch.float32, device="cuda")
...
def test_flash_attn_kvcache_nosplit(nheads_kv, gqa_ratio, num_requests, query_seqlen, context_seqlen, headdim, causal, gqa_parallel):
    ...
    out_fa3, lse_fa3 = _flash_attn_with_kvcache_out_lse(
                    q=q,
                    k_cache=k_cache,
                    v_cache=v_cache,
                    ...
                    pack_gqa=gqa_parallel
                )


    torch.cuda.synchronize()
    assert (_abs_diff(out_ref, out_fa3).max().item() <= 4e-3)
    assert (_abs_diff(out_ref, out_fa3).mean().item() <= 2e-4)
...
def test_flash_attn_kvcache_nosplit_fp8(nheads_kv, gqa_ratio, num_requests, query_seqlen, context_seqlen, headdim, causal, gqa_parallel):
    ...
    q_descale = _ones_descale(num_requests, nheads_kv)
    k_descale = _ones_descale(num_requests, nheads_kv)
    v_descale = _ones_descale(num_requests, nheads_kv)
    out_fa3, lse_fa3 = _flash_attn_with_kvcache_out_lse(
                    q=q,
                    k_cache=k_cache,
                    v_cache=v_cache,
    ...
    torch.cuda.synchronize()
    assert (_abs_diff(out_ref, out_fa3).max().item() <= 4e-2)
    assert (_abs_diff(out_ref, out_fa3).mean().item() <= 2e-3)
...
def test_flash_attn_kvcache_output(nheads_kv, gqa_ratio, num_requests, query_seqlen, context_seqlen, headdim, causal, use_heuristic_only, cache_seqlen_rand, gqa_parallel, dtype):
    ...
    out_ref, lse_ref = _flash_attn_with_kvcache_out_lse(
                    q=q,
                    k_cache=k_cache,
                    v_cache=v_cache,
                    cache_seqlens=cache_seqlens,
    ...
    for i in range(0, max_splits+1):
                ...
                print ('output-max-diff', i, context_seqlen, _abs_diff(out_ref, out_fa3).max().item())
                print ('output-mean-diff',i, context_seqlen, _abs_diff(out_ref, out_fa3).mean().item())
                print ('lse-max-diff',i, context_seqlen, _abs_diff(lse_ref, lse_fa3).max().item())
                print ('lse-mean-diff',i,  context_seqlen, _abs_diff(lse_ref, lse_fa3).mean().item())

                if cache_seqlen_rand:
                    assert (_abs_diff(out_ref, out_fa3).max().item() <= 1e-2)
                    assert (_abs_diff(out_ref, out_fa3).mean().item() <= 1e-3)
                else:
                    assert (_abs_diff(out_ref, out_fa3).max().item() <= 2e-3)
                    assert (_abs_diff(out_ref, out_fa3).mean().item() <= 1e-4)
                ...
                lse_max_diff = _abs_diff(lse_ref, lse_fa3).max().item()
                lse_mean_diff = _abs_diff(lse_ref, lse_fa3).mean().item()
...
def test_flash_attn_kvcache_output_fp8(nheads_kv, gqa_ratio, num_requests, query_seqlen, context_seqlen, headdim, causal, use_heuristic_only, cache_seqlen_rand, gqa_parallel, dtype):
    ...
    for i in range(0, max_splits+1):
                ...
                if cache_seqlen_rand:
                    assert (_abs_diff(out_ref, out_fa3).max().item() <= 1e-1)
                    assert (_abs_diff(out_ref, out_fa3).mean().item() <= 1e-2)
                else:
                    assert (_abs_diff(out_ref, out_fa3).max().item() <= 2e-2)
                    assert (_abs_diff(out_ref, out_fa3).mean().item() <= 2e-3)
                lse_max_ref = lse_ref.abs().max().item()
                lse_mean_ref = lse_ref.abs().mean().item()
                lse_max_fa3 = lse_fa3.abs().max().item()
