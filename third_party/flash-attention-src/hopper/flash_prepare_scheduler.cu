...
__global__ void prepare_varlen_num_blocks_kernel(
    ...
    auto get_q_seqlen = [&](int batch_idx) {
        int seqlen;
        if (seqused_q) {
        ...
        return batch_idx < num_batch ? seqlen : 0;
    };

    auto get_num_m_blocks = [&](int batch_idx) {
        int seqlen = get_q_seqlen(batch_idx);
    ...
    auto get_num_n_blocks = [&](int batch_idx) {
        ...
        int const q_seqlen_for_boundary = cross_kv_boundary != nullptr ? get_q_seqlen(batch_idx) : 0;
        if (cross_kv_boundary != nullptr && batch_idx < num_batch && lane < kNumBatchPerWarp) {
            int const boundary_offset = cu_seqlens_q ? cu_seqlens_q[batch_idx] : batch_idx * seqlen_q_static;
            int max_boundary = 0;
            for (int row = 0; row < q_seqlen_for_boundary; ++row) {
                max_boundary = max_boundary > cross_kv_boundary[boundary_offset + row] ? max_boundary : cross_kv_boundary[boundary_offset + row];
            }
            int const bounded_boundary = max_boundary > 0 ? max_boundary : 0;
            int const bounded_seqlen = seqlen > 0 ? seqlen : 0;
            seqlen = bounded_boundary < bounded_seqlen ? bounded_boundary : bounded_seqlen;
        }
        return batch_idx < num_batch && lane < kNumBatchPerWarp
