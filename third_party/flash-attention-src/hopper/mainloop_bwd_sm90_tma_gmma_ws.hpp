...
struct CollectiveMainloopBwdSm90 {
    ...
    to_underlying_arguments(Arguments const& args) {
        ...
        return {args.shape_Q, args.shape_K, 
                ...
                args.cu_seqlens_q, args.cu_seqlens_k, args.seqused_q, args.seqused_k, args.cross_kv_boundary};
    }
    ...
    load(Params const& params,
        ...
        int const* cross_kv_boundary_batch = params.cross_kv_boundary != nullptr
            ? params.cross_kv_boundary + ((!Varlen || params.cu_seqlens_q == nullptr) ? bidb * int(get<0>(params.shape_Q)) : seqlen_info.offset_q)
            : nullptr;
        auto [m_block_min, m_block_max] = BlockMN_t::get_m_block_min_max(
            seqlen_info, n_block, bidb,
            params.window_size_left, params.window_size_right, 0 /*sink_token_length*/, cross_kv_boundary_batch);
    ...
    store_dq(Params const& params,
        ...
        auto [m_block_min, m_block_max] = BlockMN_t::get_m_block_min_max(
            ...
            params.window_size_right, 0 /*sink_token_length*/, cross_kv_boundary_batch);
    ...
    mma(Params const& params,
        ...
        flash::Mask<kBlockM, kBlockN, false /*PackGQA*/, TiledMmaSdP, SdP_swapAB> mask(
            ...
            params.attention_chunk_divmod, params.qhead_per_khead_divmod, cross_kv_boundary_batch
        );
        ...
        static constexpr int kBlockM = get<0>(TileShape_MNK{});
        static constexpr int kBlockN = get<1>(TileShape_MNK{});
        int m_block_staircase_no_mask_start = m_block_max;
        int m_block_staircase_no_mask_end = m_block_max;
        if constexpr (Is_local && SeparateMaskingIterations) {
            if (cross_kv_boundary_batch != nullptr) {
                // For fixed K block [n_start, n_end), a Q block can use the
                // no-local-mask loop only if every in-bounds row in that
                // kBlockM tile has cross_kv_boundary[row] >= n_end. Raw cross_kv_boundary
                // can drop to 0 for right padding, so scan Q blocks and keep
                // the first contiguous fully-visible run as the fast path.
                int const n_end = std::min((n_block + 1) * kBlockN, seqlen_k);
                bool found_no_mask_block = false;
                for (int mb = m_block; mb < m_block_max; ++mb) {
                    int const row_start = mb * kBlockM;
                    int const row_end = std::min(row_start + kBlockM, seqlen_q);
                    int min_boundary = 0x7fffffff;
                    int const lane = threadIdx.x % 32;
                    for (int row = row_start + lane; row < row_end; row += 32) {
                        min_boundary = std::min(min_boundary, cross_kv_boundary_batch[row]);
                    }
                    min_boundary = BlockMN_t::warp_allreduce_min(min_boundary);
                    bool const full_block = row_start < row_end && min_boundary >= n_end;
                    if (full_block) {
                        if (!found_no_mask_block) {
                            m_block_staircase_no_mask_start = mb;
                            found_no_mask_block = true;
                        }
                        m_block_staircase_no_mask_end = mb + 1;
                    } else if (found_no_mask_block) {
                        break;
                    }
                }
                if (!found_no_mask_block) {
                    m_block_staircase_no_mask_start = m_block_max;
                    m_block_staircase_no_mask_end = m_block_max;
                }
            }
        }
        ...
        if constexpr ((Is_causal || Is_local) && SeparateMaskingIterations) {
            ...
            int const m_block_masking_max = cross_kv_boundary_batch != nullptr
                ? m_block_staircase_no_mask_start
                : ((n_block + 1) * kBlockN - 1 + seqlen_q - seqlen_k - params.window_size_right) / kBlockM + 1;
        ...
        int m_block_max_before_local_mask;
        if (!Is_local || !SeparateMaskingIterations) {
            m_block_max_before_local_mask = m_block_max;
        } else if (cross_kv_boundary_batch != nullptr) {
            // Only the scanned fully-visible Q-block run can skip the local
            // staircase mask. The remaining suffix, including a block mixed
            // with right padding rows, must go through the masked loop below.
            m_block_max_before_local_mask = m_block_staircase_no_mask_end;
        } else {
            m_block_max_before_local_mask = std::min(m_block_max, (n_block * kBlockN + seqlen_q - seqlen_k + params.window_size_left) / kBlockM);
        }
