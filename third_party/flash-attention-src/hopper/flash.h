...
struct Flash_fwd_params : public Qkv_params {
    ...
    // Per-query KV attend boundary for staircase mask (cross-attention).
    // Shape: [total_q] (varlen) or [batch, seqlen_q].
    // cross_kv_boundary[i] = number of KV tokens query position i can attend to.
    // Values are per-row prefix lengths, logically clamped to [0, seqlen_k],
    // and may be non-monotonic.
    // nullptr means no staircase mask (use normal causal/local/no mask).
    int const * __restrict__ cross_kv_boundary;
