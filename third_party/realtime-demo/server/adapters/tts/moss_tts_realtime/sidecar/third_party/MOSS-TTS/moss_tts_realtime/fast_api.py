...
# Emit audio frames incrementally as they are generated (one model step = one
# frame) rather than synthesizing the whole pushed segment before queuing any
# PCM. Cuts per-push first-audio latency; opt-in until A/B'd on the box.
STREAM_EMIT = os.getenv("MOSS_TTS_STREAM_EMIT", "").lower() in {"1", "true", "yes", "on"}
...
class SessionManager:
    ...
    def _handle_push_text(self, sess: SessionRuntime, cmd: dict) -> None:
        with sess.lock:
            ...
            if not tokens:
                return
            if STREAM_EMIT and hasattr(sess.tts_session, "push_text_tokens_iter"):
                # decode + queue PCM per frame as it is generated (low first-audio)
                for frame in sess.tts_session.push_text_tokens_iter(tokens):
                    self._emit_audio_chunks(
                        sess, sess.frame_decoder.decode_frames([frame]))
            else:
