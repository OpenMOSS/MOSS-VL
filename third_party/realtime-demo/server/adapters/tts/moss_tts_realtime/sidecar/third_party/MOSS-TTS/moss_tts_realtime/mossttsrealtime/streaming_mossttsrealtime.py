...
class MossTTSRealtimeStreamingSession:
    ...
    def push_text_tokens_iter(self, tokens: Iterable[int]) -> Iterator[torch.Tensor]:
        """Streaming form of ``push_text_tokens``: yields each audio-token frame
        the moment it is generated (one model step = one frame) instead of
        returning the whole list. Lets the sidecar decode + emit PCM
        incrementally so first-audio no longer waits for the entire pushed
        segment. Frame sequence is identical to ``push_text_tokens`` for the same
        input; purely additive (opt-in via the sidecar's MOSS_TTS_STREAM_EMIT)."""
        self._pending_tokens.extend([int(t) for t in tokens])
        yield from self._drain_pending_tokens_iter()
    ...
    def _drain_pending_tokens_iter(self) -> Iterator[torch.Tensor]:
        """Generator twin of ``_drain_pending_tokens`` — yields the prefill frame
        then one frame per decoded step, so callers stream instead of batch."""
        for frame in self._prefill_if_needed():
            yield frame
        if not self._prefilled:
            return
        while self._pending_tokens and not self.inferencer.is_finished:
            token = self._pending_tokens.pop(0)
            yield self.inferencer.step(
                token,
                temperature=self.temperature,
                top_p=self.top_p,
                top_k=self.top_k,
